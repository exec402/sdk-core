...
export function getUsdcChainConfigForChain(
  chainId: number
): ChainConfig | undefined {
  return evm.config[chainId.toString()];
}
...
export function selectPaymentRequirements(
  ...
  const broadlyAcceptedPaymentRequirements = paymentRequirements.filter(
    (requirement) => {
  ...
  const usdcRequirements = broadlyAcceptedPaymentRequirements.filter(
    (requirement) => {
      // If the address is a USDC address, we return it.
      return (
        requirement.asset ===
        getUsdcChainConfigForChain(getNetworkId(requirement.network))
          ?.usdcAddress
      );
    }
  );
